...
function CheckNames(entryData, pokemonData) {
...
for (var filePath of filePaths) {
    ...
    for (var entryData of fileData) {
        ...
        entryData.Name = FixEntryName(entryData.Name);

        CheckNames(entryData, pokemonData);
...
pokemonList.filter(p => p.EvolutionChains).flatMap(p => p.EvolutionChains).forEach(ec1 => {
    ...
    if (!evolutionChains.some(ec2 => {
        //console.log(ec1);
        testResult = ec1.Stage1DexNum == ec2.Stage1DexNum && ec1.Stage1Form == ec2.Stage1Form &&
