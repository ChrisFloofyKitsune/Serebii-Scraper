...
class PokeParser {
    ...
    GetEggMoves() {
        return this.GetNonLevelUpMoves(this.$('table.dextable').has('td.fooevo:contains("Egg")'), 1);
    }
...
class Gen3Parser extends PokeParser {
    ...
    LoadPage(pagePath) {
        ...
        this.$extraPage = cheerio.load(fs.readFileSync(`./rawHTML/generation3/extraPages/${this.dexNum}.html`));
    }
    ...
    GetLevelUpMoves() {
        ...
        */
        ...
        let result = [];
        let $levelUpTables = this.$extraPage('h4:contains("By leveling up")').nextUntil('h4').filter('table');

        $levelUpTables.each((i, table) => {
            let $e = this.$extraPage(table);
            let form = this.GetDefaultForm();

            let headerText = $e.prev().text();
            let formIndex = this.forms.findIndex(s => headerText.includes(s));
            if (formIndex != -1) {
                form = this.forms[formIndex];
            }
            ...
            result.push({
                Form: form,
                LevelUpMoves: $e.find('tbody>tr:nth-child(2) table>tbody>tr~tr').map((i, e) => {
                    let $tr = this.$extraPage(e);
                    return {
                        Name: MoveNameFix($tr.find('a').first().text()),
                        Level: $tr.find('td span').first().text().replace(/^0+/, '')
                    }
                }).get()
            });
    ...
    GetEggMoves() {
        return this.GetNonLevelUpMoves(this.$extraPage('h4:contains("By breeding")').nextUntil('h4').filter('table'), 2);
    }

    GetTutorMoves() {
        return this.GetNonLevelUpMoves(this.$extraPage('h4:contains("By tutoring")').nextUntil('h4').filter('table'), 1);
    }

    GetMachineMoves() {
        return this.GetNonLevelUpMoves(this.$extraPage('h4:contains("By TM")').nextUntil('h4').filter('table'), 3);
    }
    ...
    GetNonLevelUpMoves($tables, nameCol = 1) {
        ...
        */
        ...
        $tables.each((i, table) => {
            let $table = this.$extraPage(table);

            let form = this.GetDefaultForm();
            if (i != 0) {
                let headerText = $table.prev().text();
                let formIndex = this.forms.findIndex(s => headerText.includes(s));
                if (formIndex != -1) {
                    form = this.forms[formIndex];
                }
            ...
            $table.find('tbody>tr:nth-child(2)').nextUntil('tr:last-child').each((i, tr) => {
                let $tr = this.$extraPage(tr);
                let name = $tr.find(`td`).eq(nameCol - 1).first().text().replace(/[\*†‡]/, '');

                if (name == "")
                    return;

                name = MoveNameFix(name)
                ...
                else if (!existing.Forms.includes(form)) {
...
const defaultParser = new PokeParser();

const GenParsers = [
    { index: 1, parser: defaultParser },
    { index: 2, parser: defaultParser },
    { index: 3, parser: new Gen3Parser() },
